...
#include "stb_image.c"
		...
		const std::string& texturePath )
	{
		...
		if( !data )
		{
			...
			MessageBox( NULL, L"Error Loading File", L"File Load Error", MB_OK | MB_ICONERROR );	
			return 0;
		}
...
//			}
	...
	std::shared_ptr<Texture> Texture::CreateOrGetTexture( const std::string& texturePath )
	{
		REQUIRES( !texturePath.empty() );
		...
		if(!found)
		{	
			result = std::make_shared<Texture>(Texture( texturePath ));
