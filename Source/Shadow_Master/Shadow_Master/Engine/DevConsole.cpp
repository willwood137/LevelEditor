...
namespace shadow
{
	DevConsole* DevConsole::s_pInstance = nullptr;
...
#define DEVELOPER_CONSOLE_ENABLED
	...
	EventRecipient(eventsystem), 
		...
		m_aspect(1.0f),
		m_scroll(0), 
		m_shaderConsole(CONSOLE_SHADER_PATH),
		m_shaderCursor(CURSOR_SHADER_PATH),  
		m_enabled(true),
		m_prevInputPos(0),
		m_cursorPos(0),
		m_time(0)
		...
		if(m_enabled)
		{
			if(m_isOpen)
			{
				...
				for( size_t i = m_scroll; i >= 0 && i < numLines && y <= 1.0f - m_size; --i)
				{
					...
					Font::DrawTextToScreen(m_lines[i].s, m_font, color, Vector2f(-.99f, y), m_size );
					y += m_size;
				}

				Font::DrawTextToScreen(">" + m_currentInput, m_font, RGBA(1.0f, 1.0f, 1.0f, 1.0f), Vector2f(-.99f,.01f), m_size);
	...
	void DevConsole::catchMouseWheel( NamedPropertyContainer& parameters )
	{
		...
		{
			unsigned int wParam;
			parameters.getNamedData("Direction", wParam);
